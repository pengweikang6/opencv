...
def gray_guss(image):
    ...
    if len(image.shape) == 3:  # 检查是否为彩色图像
        gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    else:
        gray_image = image  # 如果已经是灰度图像，则直接返回
    return gray_image


# 读取待检测图片
origin_image = cv2.imread(r'E:/pythonproject/opencvcar/images/chepai/7.jpg')
...
# 接下来使用binary_image而不是gray_image进行后续处理
image = binary_image
# 显示灰度图像
plt_show(image)#3
...
image = cv2.morphologyEx(image, cv2.MORPH_CLOSE, kernelX, iterations=0)#重要此参数调试
...
iterations = 1  # 根据实际情况调整
image = cv2.dilate(image, kernelX, iterations=iterations)
image = cv2.erode(image, kernelX, iterations=iterations)
image = cv2.erode(image, kernelY, iterations=iterations)
image = cv2.dilate(image, kernelY, iterations=iterations)
...
for word in words:
    ...
    if (word[3] > (word[2] * 1.5)) and (word[3] < (word[2] * 5.5)) and (word[2] > 10):
        ...
        print(i)
print(words)
for i, j in enumerate(word_images[:7]):
    plt.subplot(1, 7, i + 1)
...
def get_chinese_words_list():
    ...
    for i in range(34, 64):
        # 将模板存放在字典中
        c_word = read_directory('E:/pythonproject/opencvcar/images/TMP/TMP/Template/Template/' + template[i])
...
def get_eng_words_list():
    ...
    for i in range(10, 34):
        e_word = read_directory('E:/pythonproject/opencvcar/images/TMP/TMP/Template/Template/' + template[i])
...
def get_eng_num_words_list():
    eng_num_words_list = []
    for i in range(0, 34):
        word = read_directory('E:/pythonproject/opencvcar/images/TMP/TMP/Template/Template/' + template[i])
